...
def main(config):
    ...
    for (y1, y2, cr, cb, gt_y, img_name) in loader:
        ...
        # 注意力
        att_module_y1 = ConvSharpnessQKVAttention(embed_dim=64, kernel_size=11, sigma=3.0).to(device)
        att_module_y2 = ConvSharpnessQKVAttention(embed_dim=64, kernel_size=11, sigma=3.0).to(device)
        ...
        for step in pbar:
            ...
            total_loss = alpha * loss_prior + 10 * loss_recon + 0.01 * loss_grad
